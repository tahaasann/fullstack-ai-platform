    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">
          ATS/AI tarama sistemlerinden gecen CV olustur ve GitHub profilini optimize et
        </p>
      </div>
      ...
      {activeTab === 'cv' && (
        <div className="space-y-6">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
            <h2 className="text-lg font-semibold text-white mb-4">ATS Gecis Kurallari</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {[
                'Standart bolum basliklari kullan (Summary, Technical Skills, Projects, Education)',
                'Her madde eylem fiili ile baslasin (Developed, Implemented, Designed, Built)',
                'Tablo, kolon, grafik KULLANMA — ATS parse edemez',
                'PDF formati (Word degil)',
                'Standart font (Arial, Calibri, Helvetica) — 10-12pt',
                'Header/footer\'a kritik bilgi KOYMA',
                'Tek sayfa (senior degilsen)',
                'Teknoloji isimlerini TAM yaz: React.js, Node.js, PostgreSQL',
                'Is ilanindaki AYNI kelimeleri kullan (synonym degil)',
                'Tarihler tutarli format: 2025 – 2026',
              ].map((rule, i) => (
          ...
          {/* CV Template */}
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
            <h2 className="text-lg font-semibold text-white mb-4">ATS-Optimized CV Sablonu</h2>
            ...
            {data.filter(p => p.cv_guidance).map((p) => (
              <div key={p.project_id} className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                ...
                >
                  ...
                  <span className="text-gray-500 text-lg">{expandedProject === p.project_id ? '−' : '+'}</span>
                </button>
                ...
                {expandedProject === p.project_id && (
                  <div className="mt-4 space-y-4">
                    ...
                    {p.cv_guidance.metrics_you_can_claim?.length > 0 && (
                      <div>
                        <div className="text-xs font-semibold text-gray-500 uppercase mb-2">Metrikler</div>
      ...
      {activeTab === 'github' && (
        <div className="space-y-6">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
            ...
            <div className="space-y-3">
              <div>
                <div className="text-xs font-semibold text-gray-500 uppercase mb-1">Bio Sablonu</div>
              ...
              {data
                ...
                .map((p) => (
                  <div key={p.project_id} className="flex items-center gap-3 bg-gray-950 rounded-lg px-3 py-2">
                    ...
                    <span className="text-xs text-gray-500 flex-1 truncate">{p.project_title}</span>
                  </div>
                ))}
            </div>
            <div className="mt-4 space-y-2">
              <div className="text-xs font-semibold text-gray-500 uppercase mb-2">Pozisyona Gore Pinleme</div>
            ...
            {data.filter(p => p.github_guidance).map((p) => (
              <div key={p.project_id} className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                ...
                >
                  ...
                  <span className="text-gray-500 text-lg">{expandedProject === `gh-${p.project_id}` ? '−' : '+'}</span>
                </button>
                ...
                {expandedProject === `gh-${p.project_id}` && (
                  <div className="mt-4 space-y-4">
                    {p.github_guidance.repo_description && (
                      <div>
                        <div className="text-xs font-semibold text-gray-500 uppercase mb-1">Repo Aciklamasi</div>
                    ...
                    {p.github_guidance.readme_sections?.length > 0 && (
                      <div>
                        <div className="text-xs font-semibold text-gray-500 uppercase mb-2">README Bolumleri</div>
                        <div className="space-y-1">
                    ...
                    {p.github_guidance.commit_examples?.length > 0 && (
                      <div>
                        <div className="text-xs font-semibold text-gray-500 uppercase mb-2">Ornek Commit Mesajlari</div>
                        <div className="space-y-1">
                    ...
                    {p.github_guidance.when_to_create && (
                      <div className="bg-blue-900/10 border border-blue-500/20 rounded-lg p-3">
                        <div className="text-xs font-semibold text-blue-400 mb-1">Ne Zaman Olustur?</div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
            ...
            <p className="text-sm text-gray-400 mb-3">Yesil contribution graph, tutarliligini ve aktifligini gosterir. Recruiter'lar buna bakar.</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {[
                'Gunde en az 3-5 anlamli commit at',
                'Bos commit atma — anlamli degisiklikler yap',
                'Hafta sonlari da commit at — tutarlilik gosterir',
                'Sadece kod degil: docs, tests, refactoring de commit sayilir',
                'GitHub Actions workflow\'lari da contribution olarak sayilir',
                'Her milestone bittiginde git tag ekle: v0.1.0, v0.2.0...',
              ].map((tip, i) => (
                <div key={i} className="text-sm text-gray-300 flex items-start gap-2">
      ...
      {activeTab === 'cover-letter' && (
        <div className="space-y-6">
          ...
          {coverLetterData?.general_tips && coverLetterData.general_tips.length > 0 && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              <h2 className="text-lg font-semibold text-white mb-4">Cover Letter Yazim Kurallari</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          {coverLetterData?.templates && coverLetterData.templates.length > 0 && (
            <div className="space-y-3">
              <h2 className="text-lg font-semibold text-white">Sablonlar</h2>
              {coverLetterData.templates.map((tmpl: CareerCoverLetterTemplate, i: number) => (
                <div key={i} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                  ...
                  >
                    ...
                    <span className="text-gray-500 text-lg">{expandedProject === `cl-${i}` ? '−' : '+'}</span>
                  </button>
          ...
          {coverLetterData?.per_project_guidance && coverLetterData.per_project_guidance.length > 0 && (
            <div className="space-y-3">
              <h2 className="text-lg font-semibold text-white">Proje Bazli Cover Letter Ipuclari</h2>
          ...
          {!coverLetterData && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 text-center text-gray-500">
              Cover letter sablonlari yukleniyor...
            </div>
          )}
        </div>
      )}
